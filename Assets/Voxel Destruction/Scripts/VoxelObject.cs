...
namespace VoxelDestruction
{
    public class VoxelObject : MonoBehaviour
    {
        ...
        private void Start()
        {
            if (buildObject)
            {
                ...
                if (BetterStreamingAssets.FileExists(path))
                {
                    ...
                    #if UNITY_WEBGL
                    StartCoroutine(LoadAsset(path, DrawVoxelFile));
                    #else
        ...
        private IEnumerator ProcessWebGLFragments(string path, Action<string> callback, Action notFoundCallback)
        {
        ...
        {
            #if UNITY_WEBGL
            StartCoroutine(ProcessWebGLFragments(path.Replace(".vox", "_" + modelIndex.ToString() + ".txt"),
        ...
        public IEnumerator LoadAsset(string path, Action<IVoxFile> callback)
        {
            UnityWebRequest wr = UnityWebRequest.Get(Path.Combine(Application.streamingAssetsPath, path));
